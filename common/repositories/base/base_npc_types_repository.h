...
public:
	...
	)
	{
		...
		if (results.RowCount() == 1) {
			...
			e.loot_lockout			= row[97] ? static_cast<int32_t>(atoi(row[97])) : 0;
			e.skip_global_loot      = row[98] ? static_cast<int8_t>(atoi(row[98])) : 0;
			e.rare_spawn            = row[99] ? static_cast<int8_t>(atoi(row[99])) : 0;
	...
	)
	{
		...
		v.push_back(columns[98] + " = " + std::to_string(e.skip_global_loot));
		v.push_back(columns[99] + " = " + std::to_string(e.rare_spawn));
